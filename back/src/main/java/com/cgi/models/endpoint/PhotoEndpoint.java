...
import com.cgi.models.beans.Photo;
import com.cgi.models.constants.PhotoConstants;
import com.cgi.models.queries.PhotoQuery;
import com.cgi.models.rest.PhotoRest;
import com.cgi.models.rest.PhotoRest.PhotoRestKey;
import com.cgi.commons.rest.domain.ManyProcessParameters;


/**
 * Web Service for {@link com.cgi.models.beans.Photo Photo} Entity.
 */
@Path("photo")
@Produces({ "application/json", "application/xml" })
@Consumes("application/json")
public class PhotoEndpoint extends AbstractEndpoint implements PhotoConstants, EndpointConstants {

	/** Logger. */
	private static final Logger logger = Logger.getLogger(PhotoEndpoint.class);
	...
	public RestResponse create(
		...
		try (RequestContext context = WsUserMgr.getInstance().getRequestContext(httpRequest)) {
			...
			Request<Photo> request = new Request<>(context, ENTITY_NAME, action, entity);
			ctrl.validate(request);
			PhotoRest restEntity = new PhotoRest();
	 ...
	 * Endpoint for action "Créer"
	 *
	 * @return The {@link com.cgi.models.rest.PhotoRest Rest Entity} if found.
	 */
	@GET
	@Path("/action/create")
	public RestResponse create(
		...
		BusinessController ctrl = new BusinessController();
		// Get entity
		PhotoRest entity = new PhotoRest();
		final Photo e;
	
		try (RequestContext context = WsUserMgr.getInstance().getRequestContext(httpRequest)) {
			checkIsActionRendered(context, action);
			Request<Photo> request = new Request<>(context, ENTITY_NAME, action);
			com.cgi.commons.ref.controller.Response<?> response = ctrl.process(request);
			e = (Photo)response.getEntity();
			if (null == e) {
	...
	public RestResponse update(
		...
		try (RequestContext context = WsUserMgr.getInstance().getRequestContext(httpRequest)) {
			...
			Request<Photo> request = new Request<>(context, ENTITY_NAME, action, pk, entity);
			ctrl.validate(request);
			PhotoRest restEntity = new PhotoRest();
			RestUtils.setRestEntity(entity, null, restEntity);
			RestResponse restResponse = new RestResponse(restEntity, context);
	...
	public RestResponse update(
		...
		try (RequestContext context = WsUserMgr.getInstance().getRequestContext(httpRequest)) {
			...
			Request<Photo> request = new Request<>(context, ENTITY_NAME, action, pk);
			com.cgi.commons.ref.controller.Response<?> response = ctrl.process(request);
			e = (Photo)response.getEntity();
			if (null == e) {
	...
	public RestResponse list(
		...
		ListCriteria<Photo> crit = new ListCriteria<>(searchCriteria, action);
		...
		try (RequestContext context = WsUserMgr.getInstance().getRequestContext(httpRequest)) {
			checkIsActionRendered(context, action);
			checkIsListRendered(context, Query.PHOTO);
			ListData data = ctrl.getListData(ENTITY_NAME, Query.PHOTO, crit, context);
	...
	/**
	 * Endpoint for query "PHOTO"<br/>
	 *
	 * @return List Photo of result.
	 */
	@GET
	@Path("/query/photo")
	public RestResponse photoList(
		@QueryParam("order-by") String orderByName,
		@QueryParam("order-direction") String orderDirection,
		@QueryParam("start-index") @DefaultValue("0") int startIndex,
		@QueryParam("length") @DefaultValue("200") int length,
	...
	/**
	 * Endpoint for retriving an Photo from its children
	 *
	 * @return The {@link com.cgi.models.rest.PhotoRest Rest Entity} if found.
	 */
	@GET
	@Path("/id/{pk}/back-ref/{link-name}")
	public PhotoRest getFromBackRef (
		@PathParam("pk") PhotoRestKey pk,
	...
	public RestResponse getQueryCounts(
		...
		return super.getQueryCounts(ENTITY_NAME, PhotoQuery.QUERIES.keySet(), queryNames, httpRequest);
	}
